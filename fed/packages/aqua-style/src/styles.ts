import type { Simplify } from "@simplex/shared";
import {
  EMPTY_OBJECT,
  isArray,
  isFunction,
...
export function isStylesType(target: any): target is Styles<any, any> {
  ...
  return isFunction(candidate) && isObject(candidate.definition) && isObject(candidate.classes);
}
  ...
  >(() => {
    ...
    >(() => {
      const compoundVariants =
        ...
        (isArray(styles.compoundVariants) ? styles.compoundVariants : [styles.compoundVariants]);
        ...
        for (const compoundVariant of compoundVariants) {
          const isMatched = Object.entries(compoundVariant.variants).every(
            ([compoundVariantName, compoundVariantValue]) => {
              if (isUndefined(compoundVariantValue)) {
                return true;
              }

              const configVariantValue = configVariantsValue[compoundVariantName];

              if (isUndefined(configVariantValue)) {
                return false;
              }
              ...
              if (!isArray(compoundVariantValue)) {
